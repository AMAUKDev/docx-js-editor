 ...
 *
 * Ported from eigenpal/docx-editor upstream/main (includes fixes from
 * 10fe14d, 04b470b, 3296811).
 */
 ...
 * Pass initialActive=true when the editor starts in 'suggesting' mode.
 */
...
export function toggleSuggestionMode(
  ...
  if (dispatch) {
    dispatch(state.tr.setMeta(suggestionModeKey, { active: !current.active }));
  }
  return true;
}

/** Set suggestion mode active state and optional author. */
export function setSuggestionMode(
  ...
  if (dispatch) {
    ...
    dispatch(state.tr.setMeta(suggestionModeKey, meta));
  }
  return true;
}
